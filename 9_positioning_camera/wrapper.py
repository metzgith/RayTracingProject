...
class Camera():
    def __init__(self, vfov=90, aspect_ratio=(16 / 9), lookfrom=Point([0, 0, 0]), lookat=Point([0, 0, -1]),
                 vup=Vector([0, 1, 0])):
    ...
    def get_ray(self, h, w):
        direction = self.lower_left_corner + w * self.horizontal + h * self.vertical - self.origin
...
class Sphere():
    def __init__(self, center=Point([0, 0, -1]), radius=0.5, material=Material(color=Color([1, 1, 1]))):
        self.center = center
        self.radius = radius
